...
export class CurrencyMismatchError extends MonetaError {
  ...
  constructor(expected: string, received: string) {
    super(
      `Currency mismatch: cannot operate on ${expected} and ${received}. ` +
...
export class InvalidAmountError extends MonetaError {
  ...
  constructor(input: unknown) {
    super(
      `Invalid amount: "${String(input)}". ` +
        'Expected a numeric string (e.g. "19.99"), a number, or a bigint.',
    )
  ...
  constructor(currency: string) {
    super(
      `Unknown currency code: "${currency}". ` +
        'See the CurrencyCode type for supported currencies.',
    )
